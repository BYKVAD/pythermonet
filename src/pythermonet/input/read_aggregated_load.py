...
class AggregatedLoadInput:
    ...
    temperature_delta_heating: float          # brine ΔT across HP, heating mode [K]
    ...
    eer: float
    temperature_delta_cooling: float          # brine ΔT across HP, cooling mode [K]
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "has_cooling", (
            self.load_peak_cooling > 0.0
            and self.eer > 0.0
            and self.temperature_delta_cooling > 0.0
        ))
...
def read_aggregated_load_tsv(path: str | Path) -> AggregatedLoadInput:
    ...
    return AggregatedLoadInput(
        ...
        load_annual_heating=_f("Yearly_heating_load_(W)"),
        load_winter_heating=_f("Winter_heating_load_(W)"),
        load_peak_heating=_f("Daily_heating_load_(W)"),
        cop_annual_heating=_f("Year_COP"),
        cop_winter_heating=_f("Winter_COP"),
        cop_peak_heating=_f("Hour_COP"),
        temperature_delta_heating=_f("dT_HP_Heating"),
        n_consumers_cooling=n_cool,
        load_annual_cooling=load_y_c,
        load_summer_cooling=load_s_c,
        load_peak_cooling=load_p_c,
        eer=eer,
